...
- [x] Create OpenAI / Groq API key — config updated with `openai_base_url` for Groq-compatible endpoint
...
- [x] `app/main.py` — FastAPI app, CORS, health check (`GET /health`) — no routers mounted yet
- [x] SQLAlchemy models in `app/database/`:
  - [x] `profiles` (tied to Supabase auth.users)
  - [x] `source_documents`
...
- [x] Alembic init + first migration:
  ...
  - [x] GIN index (full-text) + HNSW index (vector)
- [x] RLS policies (users see only their own chats; public corpus readable by authenticated users)
- [x] `uv run alembic upgrade head` against Supabase direct connection
- [x] `app/database/supabase.py` — user-scoped and service-role clients (async)
...
| Doc                                                  | Purpose                                       |
| ---------------------------------------------------- | --------------------------------------------- |
| [client-brief.md](client-brief.md)                   | What Driftwood needs and e ample questions    |
